  ...
  return (
    <footer className="bg-gray-800">
      <div className="px-4 py-12 mx-auto overflow-hidden max-w-7xl sm:px-6 lg:px-8">
        <ul className="flex flex-wrap justify-center -mx-5 -my-2" aria-label="Footer">
          {navigationMain && navigationMain.map((item) => (
            <li key={item.name} className="px-5 py-2">
              <Link className="text-base text-gray-300 hover:text-white" href={item.href}>
                {item.name}
              </Link>
            </li>
          ))}
        ...
        <p className="mt-8 text-base text-center text-gray-400">Site conçu par l'équipe <Link className="hover:text-gray-50" href="https://lasercats.fr">Lasercats</Link></p>
      </div >
    </footer >
